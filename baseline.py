...
# Import ryu.topology.switches BEFORE calling set_override so that the
# 'topology' option group is already registered (register_opts runs at module
# import time in ryu/topology/switches.py). Without this import the group
# doesn't exist yet, set_override raises and is swallowed, and LLDP stays off.
import ryu.topology.switches  # noqa: F401
try:
    # observe-links is registered via register_cli_opts in ryu/topology/switches.py
    # (no group). oslo_config accepts the underscore form for the hyphenated name.
    ryu_cfg.CONF.set_override('observe_links', True)
except Exception:
    pass
...
class BaselineReactiveController(app_manager.RyuApp):
    ...
    Design goals:
      ...
      - Emits benchmark-friendly log tokens:
            ...
            BASELINE_RPPT_MEASURED: key=(...) elapsed_ms=X.XXX
    ...
    # Set to 2/2 for simpletp.py, or 23/44 for the large industry topology
    EXPECTED_SWITCHES = 2
    ...
    def __init__(self, *args, **kwargs):
        ...
        # script writes (campus_topo.py), so TOPO_DISCOVERY_COMPLETE fires for any
        # topology size without editing constants. The file may not exist yet at
        # startup (controller launched before the topology finishes), so this is
        # retried in _maybe_complete_discovery until it loads once.
    ...
    def _load_topology_expectations(self) -> bool:
        ...
        baseline adapts to any topology size/shape without code changes (same
        mechanism as the MTD controller, for a fair comparison).
        """
    ...
    def _maybe_complete_discovery(self, reason: str):
        # Retry loading expectations in case the controller started before the
        # topology wrote the file.
        if not self._expectations_loaded:
            self._load_topology_expectations()
    ...
    def _refresh_topology(self, reason: str):
        ...
        for link in links:
            # Record BOTH directions from each directed link object so adj is always
            # symmetric. ryu.topology emits each physical link as two directed Links,
            # but they can arrive at different times; relying on the reverse Link to
            # show up leaves adj asymmetric in the interim, which makes _path_ports
            # raise KeyError when it looks up the reverse (in_port) hop. Deriving the
            # reverse edge from the same link (dst.port_no) closes that gap.
            new_adj.setdefault(link.src.dpid, {})[link.dst.dpid] = link.src.port_no
            new_adj.setdefault(link.dst.dpid, {})[link.src.dpid] = link.dst.port_no
        self.adj = new_adj
    ...
    def _learn_host(self, pkt, dpid: int, in_port: int, eth) -> Optional[str]:
        ...
        # If we already have a confirmed location for this host on a DIFFERENT
        # switch, this PacketIn is a flooded copy arriving via an inter-switch
        # link whose uplink status isn't known yet (self.adj still empty at
        # startup). Ignore it — overwriting would corrupt the host location and
        # cause NO_PATH errors until LLDP populates self.adj.
        existing = self.host_location.get(real_ip)
        if existing is not None and existing[0] != dpid:
            return real_ip
    ...
    def _install_path_and_forward(self, msg, dp, in_port, eth, ip4, pkt, dst_loc, rppt_start):
        ...
        self.logger.info("BASELINE_RPPT_MEASURED: key=%s elapsed_ms=%.3f", key, elapsed_ms)
        self.logger.info(
