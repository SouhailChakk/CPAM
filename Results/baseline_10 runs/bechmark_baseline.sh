#!/usr/bin/env bash
# =============================================================================
#  Baseline Reactive Controller — NSDI Benchmark Script  (v3-baseline)
#
...
#    Good for: FSR (N1), RPPT under load (N3).
...
#      [C]   TCP Session Continuity   — long-lived flow stability
...
#      [N4]  Flow Setup Latency       — pathsw/flowmods stats from FLOW_SETUP log
#      [N5]  Scalability Snapshot     — structured per-topology summary
#                                       for cross-scale comparison
#
#  Usage:
#    sudo ./bechmark_baseline.sh [DURATION] [LABEL] [RYU_LOG] [TOPOLOGY]
#
...
#
#  Examples:
#    sudo ./bechmark_baseline.sh 120 baseline_small  /var/log/ryu.log small
#    sudo ./bechmark_baseline.sh 120 baseline_medium /var/log/ryu.log medium
#    sudo ./bechmark_baseline.sh 120 baseline_large  /var/log/ryu.log large
#
#  10-run loop per topology (RFC 8456 §4.7):
#    for TOPO in small medium large; do
#      for i in {1..10}; do
#        sudo ./bechmark_baseline.sh 120 baseline_${TOPO}_iter${i} \
#             /var/log/ryu.log ${TOPO}
#      done
#    done
#
#  Controller log tokens emitted by baseline.py:
#    TOPO_DISCOVERY_COMPLETE reason=X elapsed=X.XXX switches=N directed_links=N hosts=N
#    RPPT_MEASURED: key=(...) elapsed_ms=X.XXX
#    FLOW_SETUP: key=(...) pathsw=N flowmods=N elapsed_ms=X.XXX
# =============================================================================
...
if [[ "$ITERATIONS" -gt 1 ]]; then
    ...
    echo ""
    echo "[DONE] Completed $ITERATIONS iterations — results in: cpam_baseline_${LABEL}_iter*"
    exit 0
fi

LONG_DUR="$((DUR * 2))"
TS="$(date +%Y%m%d_%H%M%S)"
OUTDIR="cpam_baseline_${LABEL}_${TS}"
mkdir -p "$OUTDIR"
...
cat << BANNER
╔══════════════════════════════════════════════════════════════════╗
║     Baseline Reactive Controller — NSDI Benchmark v3-baseline    ║
╠══════════════════════════════════════════════════════════════════╣
║  Label      : ${LABEL}
║  Topology   : ${TOPO_LABEL}
...
║  Ryu log    : ${RYU_LOG}
║  Output     : ${OUTDIR}
║  Controller : baseline.py (no VIPs, real IPs used for all traffic)
╠══════════════════════════════════════════════════════════════════╣
...
# ── Baseline: no VIP mapping — all traffic uses real IPs directly ──────────────
# baseline.py does not assign VIPs; HOST_VIPS is set equal to HOST_IPS so the
# rest of the script (which references HOST_VIPS for dst addresses) works unchanged.
VIP_MAPPING=""
...
VIP_LOOKUP_HELPER="/tmp/baseline_ip_passthrough.py"
ensure_vip_lookup_helper() {
    cat > "$VIP_LOOKUP_HELPER" <<'PY'
#!/usr/bin/env python3
import sys
# Baseline has no VIP mapping — return the real IP unchanged.
print(sys.argv[1] if len(sys.argv) > 1 else "")
PY
    chmod +x "$VIP_LOOKUP_HELPER" 2>/dev/null || true
}

echo "[INIT] Baseline mode — no VIP mapping; all traffic uses real IPs."
for hname in "${!HOST_IPS[@]}"; do
    HOST_VIPS["$hname"]="${HOST_IPS[$hname]}"
done
...
if [[ "$TOPOLOGY" == "small" ]]; then
    for i in $(seq 0 $((NUM_PAIRS - 1))); do
        src="${PAIRS_SRC[$i]}"
        dst="${PAIRS_DST[$i]}"
        echo "  $src (${HOST_IPS[$src]}) -> $dst (${HOST_IPS[$dst]})"
    done
fi
echo ""
...
# ── Helpers ───────────────────────────────────────────────────────────────────
refresh_vips() {
    # Baseline: no VIP mapping — nothing to refresh.
    true
}
...
preflight_controller_tokens() {
    ...
    if grep -q 'FLOW_SETUP' "$RYU_LOG" 2>/dev/null; then
        echo "[PREFLIGHT] OK: FLOW_SETUP"
    else
        echo "[PREFLIGHT][WARN] FLOW_SETUP not seen during warm-up (metric N4 may be empty)"
    fi
...

{
    ...
    if [[ -n "$RYU_LOG" && -f "$RYU_LOG" ]]; then
        ...
        if [[ -n "$DISCO_LINE" ]]; then
            ...
            echo "Hosts discovered    : ${DISCO_HOSTS}"
        else
...
N1_LOG="${OUTDIR}/N1_flow_setup_rate.txt"
{
    ...
    if (( ELAPSED_CHURN_MS > 0 )); then
        ...
        echo "  Compare small/medium/large to show how FSR scales with topology."
...
# =============================================================================
log_step "[C] TCP Session Continuity — Model 1 — ${LONG_DUR}s long-lived flows"
refresh_vips
stop_all_iperf
...
echo "[C] ${#LONG_TCP_PIDS[@]} sessions for ${LONG_DUR}s"
...

{
    ...
    [[ "$BROKEN" -eq 0 ]] && \
        echo "RESULT: PASS — all sessions completed successfully" || \
        echo "RESULT: FAIL — $BROKEN session(s) failed"
...
if [[ -n "$RYU_LOG" && -f "$RYU_LOG" ]]; then
    {
        echo ""
        echo "=== Baseline Controller Events During NRT Window ==="
        tail -n +$((RYU_LOG_START + 1)) "$RYU_LOG" 2>/dev/null | \
            grep -E 'TOPO_DISCOVERY_COMPLETE|TOPOLOGY_REFRESH|NO_PATH|EventLink' | head -20 || \
            echo "No topology events found in log"
    } >> "$NRT_LOG"
fi
...
# =============================================================================
# [N4] FLOW SETUP LATENCY (baseline-specific: parsed from FLOW_SETUP log token)
# =============================================================================
log_step "[N4] Flow Setup Latency — path length + flowmod stats"

N4_LOG="${OUTDIR}/N4_flow_setup_latency.txt"
{
    echo "=== [N4] Flow Setup Latency (baseline.py FLOW_SETUP token) ==="
    echo "Topology : $TOPO_LABEL"
    ...
    if [[ -n "$RYU_LOG" && -f "$RYU_LOG" ]]; then
        ...
        FS_LINES="$(echo "$NEW_LINES" | grep 'FLOW_SETUP:' || true)"
        ...
        if [[ "$FS_COUNT" -gt 0 ]]; then
            ...
            echo ""
            echo "INTERPRETATION:"
            echo "  elapsed_ms = time from first PacketIn to flow install (= RPPT)"
            echo "  pathsw     = number of switches on the forwarding path"
            echo "  flowmods   = forward + reverse flow entries installed per setup"
        else
            echo "No FLOW_SETUP events found in this run window."
            echo "Token emitted by baseline.py on every new flow installation."
        fi
    else
        echo "Ryu log not available."
    fi
...
N5_LOG="${OUTDIR}/N5_scalability_snapshot.txt"
{
    ...
    echo "Flow setup latency (avg path / avg flowmods):"
    grep -E 'Avg path length|Avg flow mods' "$N4_LOG" 2>/dev/null | sed 's/^/  /' \
        || echo "  see N4_flow_setup_latency.txt"
    echo ""
    ...
    echo "  Avg path length     ?      ?       ?      -> grows with topology depth"
...
SUMMARY="${OUTDIR}/SUMMARY.txt"
{
    cat << EOF
╔══════════════════════════════════════════════════════════════════╗
║   Baseline Reactive Controller — NSDI Benchmark — Master Summary ║
╠══════════════════════════════════════════════════════════════════╣
║  Label      : ${LABEL}
║  Topology   : ${TOPO_LABEL}
║  Timestamp  : ${TS}
...
║  Controller : baseline.py (real IPs, no VIPs)
...
║  [N4]   Flow Setup Latency (baseline): N4_flow_setup_latency.txt
...
║  for TOPO in small medium large; do                              ║
║    for i in {1..10}; do                                         ║
║      sudo ./bechmark_baseline.sh ${DUR} baseline_\${TOPO}_iter\${i} \\ ║
║           ${RYU_LOG:-/var/log/ryu.log} \${TOPO}                          ║
║    done                                                          ║
║  done                                                            ║
║                                                                  ║
║  CONTROLLER LOG TOKENS (baseline.py):                            ║
║    TOPO_DISCOVERY_COMPLETE reason=X elapsed=X.XXX -> [1]        ║
║    RPPT_MEASURED: key=(...) elapsed_ms=X.XXX      -> [3] N2 N3  ║
║    FLOW_SETUP: key=(...) pathsw=N elapsed_ms=X    -> N4         ║
╚══════════════════════════════════════════════════════════════════╝
EOF
} > "$SUMMARY"
